...
function PopUp({ onClose }) {
  return ReactDOM.createPortal(
    <div className="fixed top-0 left-0 w-full h-full bg-black bg-opacity-50 flex justify-center items-center z-50">
      <div className="rounded-[40px] w-4/5 bg-black flex flex-wrap text-white">
        <div className="flex items-center justify-center w-full h-auto py-10 px-5 lg:w-1/2 lg:h-[600px] relative ">
          <div className="flex flex-col items-start justify-center w-full lg:w-[500px]">
            <img
              className="rounded-[40px] w-full h-full object-cover absolute top-0 left-0"
              src={bgForm}
              alt=""
            />
            ...
            <button
              onClick={onClose}
              className="mt-4 p-2 text-white z-50 absolute right-5 top-0">
